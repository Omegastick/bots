...
{
QuadScreen::QuadScreen(std::vector<std::shared_ptr<IScreen>> *screens, std::vector<std::string> *screen_names)
    : screens(screens), screen_names(screen_names)
{
...
void QuadScreen::update(const float delta_time)
{
    ...
    ImGui::Begin("Screens");
    int screen_index = std::find(screen_names->begin(), screen_names->end(), "Quad test") - screen_names->begin();
